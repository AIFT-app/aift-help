...
AI Finance Team recognises these automatically and **does not** rely on the description wording alone. When the other account on a transaction is one of your own company's bank accounts (matched by IBAN or account number against your [companies'](/master-data-entities) bank accounts), the transaction is:

- given the **Internal transfer** type,
- marked **No invoice needed**, and
- linked to a special **Own company** partner rather than a junk supplier record (see [Managing Partners](/partners#internal-transfers-between-your-own-accounts)).

Because this is based on your saved account numbers - not forgeable text - keep each company's bank accounts up to date under **Master data → Companies** so transfers are caught reliably. A transaction whose description merely mentions a transfer word (e.g. "átvezetés") but whose other account you don't own is still flagged **No invoice needed** with the Internal transfer type, but it is **not** linked to an own company.

...
Both lists live on the Master data → **Accounting categories** page, one tab each (**Invoice**, **Cash**).
...
AI Finance Team does not come with a ready-made category list. Every office keeps its own chart of accounts, and a generic list would only have to be undone. So a new workspace has **no invoice categories and no cash categories** until your accountant brings in the office's own list:

- **From a spreadsheet** - Master data → **Accounting categories**, open the **Invoice** or **Cash** tab and click **Import**. See [Importing master data](/master-data-import).
- **From Business Central** - when the client's Business Central is connected, its chart of accounts can be imported as invoice categories.
- **By hand** - add categories one by one on the same tabs.

Until a list exists, nothing is categorised: invoices and bank transactions still arrive, match and appear in exports, just without a category. Once the list is in place, select the records on the **Invoices** or **Bank transactions** page and use **Re-categorize** to let the AI book the ones that arrived earlier.
...
Accountants can see, edit, and pre-load what the AI has learned on the **Examples** tab of the Accounting categories page - see [How AIFT Learns Your Categories](/categorization-examples).

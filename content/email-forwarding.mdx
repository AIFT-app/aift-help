...
> The address is **per workspace**, not per company or per person. Anything sent to it lands in that one workspace's Intake.
...
Managing aliases (claiming or retiring) is limited to **accountant admins** and **finance leads**. Everyone else sees the addresses read-only - which is all you need to forward.
...
3. Invoices and receipts flow into extraction and show up on the **Invoices** page like any other upload. A bank statement is **not** extracted this way: it waits in Intake marked **Awaiting extractor**, and has to be uploaded on the **Bank transactions** page to be read, see [Uploading Bank Statements](/bank-statement-upload).

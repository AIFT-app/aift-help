...
- This feature is available only for companies whose country is **Hungary (HU)**. Non-Hungarian companies will not see the NAV section.
- The connection can be set up by a **finance lead** or an **accountant admin**. (In practice accounting offices usually create the NAV technical user themselves, so they can start the connection too.) Other roles can see the connection *status* but never the credentials.
- You need a **NAV technical user** ("technikai felhasználó"), which you create inside your own NAV account. Specifically, AI Finance Team needs:
  - the company's **tax number** (the 8-digit core),
...
1. From the workspace sidebar, open **Master data → Companies** and select your Hungarian company.
...
- **Income vs. expense is set from NAV's direction**: invoices NAV reports as *outbound* for your company are income; *inbound* are expenses.
...
- If no invoices appear after connecting, confirm the technical user has the **invoice query** permission in NAV and that the company's tax number is correct.

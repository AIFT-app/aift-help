...
2. **AI semantic scoring** — Claude reviews each candidate pair and assigns a confidence score (0–100) based on invoice number in the transaction reference, counterparty name, amount proximity, and date. When the system has already verified that both sides point to the same counterparty (same ID, same verified partner, or the same legal name after stripping suffixes), that signal is locked in — so a clean structural match doesn't drift in the score band between runs.
   ...
   **Targeted threshold relaxation:** when every structural signal lines up (same counterparty by id, exact amount on the chosen currency path, transaction within 30 days of the invoice's due date, no anomalous mention of your own entity in the bank text), the AI's score is floored at 85 even without the invoice-number signal. So a payment that obviously corresponds to a single open invoice will auto-match instead of landing in the medium-confidence band for manual review. The relaxation is strict: name-only counterparty matches, partial amounts, and pairs farther than 30 days apart do not qualify.
...
Go to **Matching** in the workspace sidebar. The page shows a **single unified list** of every match in the workspace — both pending AI suggestions and already-confirmed matches — newest first. (There are no longer separate "Pending suggestions" and "All matches" sections; everything lives in one list.)

Each row has up to four columns:
...
The search box filters the list by **invoice number**, **counterparty name**, or **payment reference**. It searches as you type. The raw bank-description free text is *not* searched — use the structured reference instead. Your search is reflected in the URL (`?search=INV-2026-014`) so you can bookmark or share it.

### Filtering

Two filters sit below the search box:
...
- **Confidence** — toggle the **High**, **Medium**, and **Low** pills to include or exclude each level. All three are on by default. For pending rows the confidence band is derived from the AI score (High ≥ 85, Medium ≥ 60, Low below 60).
...
Choose how many rows to show per page with the **Per page** selector (**10**, **50**, or **100**; default 50). Your choice is remembered across visits. Use **Previous** / **Next** to move between pages. The pagination controls only appear when there is more than one page of results.
...
- When you receive a vendor invoice for a card payment you've already made, expect the match to surface as a suggestion rather than auto-match. The amount + counterparty + date will line up; you just need to confirm.
- If you regularly pay the same vendor by card, ask the vendor to keep the merchant-name string they send to acquirers stable, or merge the alias on the Partners page once it shows up there.

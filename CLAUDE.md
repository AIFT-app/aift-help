...
- **Phones** (figure under 640px): the screen is laid out at 640px and zoomed down (about 60% on a 390px phone, too small to read), so `zoomable` adds an **Enlarge** control that opens it full screen at 100% to swipe or pinch around. Inside `.app-screen` the `sm:` and `pointer-fine:` variants always apply (`globals.css`), so a phone shows the desktop screen, not a mix of desktop layout and Catalyst's phone variants. The numbered legend under each screen stays the readable version on a phone.
